...
```{r, echo=F}
# 
EPA.Air.Quality <- data.frame(ItemName = c("Source", "Data", "Filename"),
  Value = c("EPA Air Quality System (AQS)", "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv"))
colnames(EPA.Air.Quality) <- c("Item Name", "Value")

knitr::kable(EPA.Air.Quality, caption = "EPA Air Quality", align = c("r", "l"))
```
...
```{r project.setup, message=F, warning=F, echo=T}
#Import libraries
...
```{r data.summary, include=F, warning=FALSE}
...
# pulling the month.abb package in r to assign month with name
epa_data$Month.factor <- factor(epa_data$Month, levels = 1:12, 
  labels=month.abb)

Ozone.month.2018 <- 
  ggplot(subset(epa_data, Year == "2018"), 
              aes(x=Month.factor, y=Ozone)) +
  geom_boxplot() +
  xlab("Month")+
...
```
```{r, echo=F, warning=FALSE, fig.align='left', fig.height=3, fig.width=6, fig.cap="2018 Ozone values by month"}
Ozone.month.2018
```
```{r, echo=F, warning=FALSE, fig.align='left', fig.height=3, fig.width=6, fig.cap="2019 Ozone values by month"}
Ozone.month.2019
```
...
https://github.com/Hopefullyxr/EDA_Spring2024.git